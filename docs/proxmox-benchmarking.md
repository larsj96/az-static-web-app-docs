...
Create a repeatable way to measure VM performance across the HP Proxmox nodes before and after storage/network changes.
...
| VM | Proxmox node | VMID | CPU | RAM | Disk |
| --- | --- | ---: | ---: | ---: | ---: |
...
```mermaid
flowchart LR
  tf["Terraform apply\nproxmox-bench"] --> vms["bench-hp1/2/3\none per HP node"]
