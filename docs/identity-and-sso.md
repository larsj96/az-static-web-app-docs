...
| Vault secret path | `homelab/authentik/bootstrap` |
...
Bootstrap credentials are generated by Ansible, written root-only on `auth1` at `/opt/authentik/credentials.txt`, and mirrored into Vault KV at `homelab/authentik/bootstrap`. Use the `akadmin` account only for bootstrap and recovery; daily users should be named users with TOTP MFA.

The Ansible role now also bootstraps named local users and groups so Authentik is usable immediately after redeploy:

| Object | Purpose |
| --- | --- |
| `larsj96` | Named admin user for Lars |
| `jaguni` | Named non-admin user |
| `homelab-admins` | Homelab admin grouping |
| `homelab-users` | Normal homelab users |
| `homelab-friends` | Future external/friend access grouping |

`larsj96` is also placed in the built-in `authentik Admins` group. `jaguni` is placed in `homelab-users`.

Managed local user passwords are generated once by Ansible on the control host and written root-only on `auth1`:

```bash
ssh ubuntu@10.0.0.36 'sudo cat /opt/authentik/credentials.txt'
```

Do not use these as long-term human passwords. First login should be followed by changing the password and enrolling TOTP MFA.

Cloudflare Tunnel/reverse-proxy note: Authentik is exposed through Cloudflare Tunnel and therefore must trust proxy headers from private tunnel/origin ranges. The Ansible role sets `AUTHENTIK_LISTEN__TRUSTED_PROXY_CIDRS` for the homelab private networks.
